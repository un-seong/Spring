...
public class StudentController {
	...
	public String ex02(StudentDTO dto) throws Exception {
		...
		return "redirect:/ex01";	//포워드 대신 /ex01주소로 리다이렉트한다
	...
	@PostMapping("/ex04")	// 먼저 찾기
	public ModelAndView ex04(String name) {
		ModelAndView mav = new ModelAndView("/ex04");
		StudentDTO ob1 = studentService.selectOne(name);
		mav.addObject("ob1", ob1);
		return mav;
	}
	
	@GetMapping("/ex04result")
	public void ex04result() {}
	
	@PostMapping("/ex04result")
	public String ex04result(StudentDTO user) {
		int row = studentService.update(user);
