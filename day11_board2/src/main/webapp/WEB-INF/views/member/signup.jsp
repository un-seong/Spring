...
<form method="POST">
	<fieldset style="flex-direction: column;">
		<textarea class="agreement"  style="resize: none" cols="20" rows="20">${agreement }</textarea>
			<p>
				<label>
					필수 약관을 모두 읽었으며, 이에 동의 합니다
					<input type="checkbox" name="agreement" disabled required>
				</label>
			</p>
	</fieldset>
	<p>
		<input type="text" name="userid" placeholder="ID입력하세요" autocomplete="off" required autofocus>
		<input type="button" name="dupCheck" value="중복체크"><br>
		<span class="dupResult"></span>
	</p>
	<p><input type="password" name="userpw" placeholder="비밀번호 입력하세요" required></p>
...
<script>
	...
	dupCheck.onclick = function(event) {
		...
		.then(resp => resp.text())		// 응답(1:이미사용중인 ID 입니다)이 돌아오면, 응답을 문자열로 처리하여
